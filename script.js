...
function inputChange(id){
    ...
    i = idList[1]
    j = idList[2]
    var jFloor = Math.floor(j/2);
    var jmod = j%2;
    console.log(jFloor)
    console.log(jmod)
    if (jmod){
        days[0][arrayVariable[i]].delivery.timings[jFloor].open = inputObj.value
    }
    else{
        days[0][arrayVariable[i]].delivery.timings[jFloor].close = inputObj.value
    }
    days[0][arrayVariable[i]].is_same = false;
      ...
      // creating vertical divisions//
        ...
        namecolumn.appendChild(heading)
            ...
            for (j=0;j<day_object.delivery.timings.length*2;j++){
                ...
                inputField.id = "delinput_"+i+"_"+j
                inputField.setAttribute('onChange','inputChange(id)')
            }
